...
function initFilesExplorer(){
  ...
  function renderFolder(folder){
    ...
    Object.entries(folder).forEach(([name, node]) => {
      ...
      el.textContent = (node.type === 'dir' ? '📁 ' : '📄 ') + name;
...
function renderFilesManagerOnce() {
  ...
  function renderTree() {
    ...
    const lines = Object.keys(children).sort((a,b) => {
      ...
      if (ch.type === 'dir') {
        return `<div class="tree-row folder" data-name="${name}">📁 ${name}</div>`;
      } else {
        return `<div class="tree-row file" data-name="${name}">📄 ${name}</div>`;
      }
    }).join('');
  ...
  function showFile(pathArr) {
    ...
    if (name.endsWith('.html') || name.endsWith('.htm')) {
      ...
      previewEl.innerHTML = `<img src="../../assets/images/${name}" alt="${name}" style="max-width:100%; border-radius:8px"/>`;
    } else {
...
(function setupHeroVisibility() {
  ...
  const statsObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        // Gdy STATS wchodzi w ekran — chowamy HERO
        if (entry.isIntersecting) {
          heroMain.classList.add("hidden");
          heroHidden = true;
        } else {
          // Pokaż HERO tylko jeśli wychodzimy z STATS w górę (nie w dół)
  ...
  // Ukrywanie HERO przy wejściu w Projects lub Tools
...
// Dodatkowo: nasłuch scrolla, by płynnie reagować przy szybkim przewijaniu
window.addEventListener('scroll', () => {
  ...
  // Gdy jesteśmy z powrotem powyżej STATS → pokaż HERO z powrotem
  if (aboveStats) {
    heroMain.classList.remove('hidden');
  }
});
...
// Obsługa kliknięć promptów (4 przyciski)
  ...
  return new Promise((resolve) => {
    ...
    function typeNext() {
      ...
      if (i <= text.length) {
        ...
        // Upewnij się, że tylko ostatnia linia ma klasę .typing
